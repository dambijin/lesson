...
// 	let enames = document.querySelectorAll("[data-ename^=ename_]")
	...
	let empno = document.querySelectorAll("empno");
	empno.addEventListener("click", (event)=>{
		// 만약 이걸 클릭하면 그에 해당하는 sql문을 order by 해준다
		// 근데 어케함? 진짜 모름
		// 주석에 다적었으니까 다했다 완료 쨋든 다함.
...
<body>
	...
	<form method="get" action="dynamic">
		...
		<!-- 정렬용 -->
		<input type="hidden" name="orderColumn" value=""> <input
			type="hidden" name="orderType" value="">
	</form>
	<form method="get" action="dynamic">
	<input type="submit" value="부분조회">
		<table border=1>
			<thead>
				<tr>
					<th>체크</th>
					<th id="empno" data-orderType="${orderType }"><c:if
							test="${orderColumn == 'empno' }">
						...
						</c:if> <c:if test="${orderColumn != 'empno' }">
	               empno
	            </c:if></th>
					<th>ename</th>
					<th>sal</th>
					...
					<c:forEach var="dto" items="${list }">
						<tr>
							...
							<td>
								...
								</form> <span class="ename" id="ename_${dto.empno }"
								data-ename="ename_${dto.empno }">${dto.ename }</span>
							</td>
							<td>${dto.sal }</td>
							<td>${dto.job }</td>
							<td>${dto.deptno }</td>
